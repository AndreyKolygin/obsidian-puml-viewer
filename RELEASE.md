...
This project is prepared for release `0.2.0`.
...
Important:
- Tag name must exactly match `manifest.json` version (`0.2.0`).

## 3) Create GitHub Release

- Create release from tag `0.2.0`.
- Upload assets:
  - `main.js`
  - `manifest.json`
  - `styles.css`
...
```json
{
  ...
  "description": "Render .puml files as PlantUML diagrams inside Obsidian.",
...
- `manifest.json` has:
  ...
  - `version` matching release tag (`0.2.0`)
